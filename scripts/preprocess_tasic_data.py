...
from cnmf import get_highvar_genes, load_df_from_npz, save_df_to_npz
...
counts = pd.read_csv(
    "./data/Part3_VisualCortex/GSE71585_RefSeq_counts.csv", index_col=0
).T

counts_per_kb = rpkm.multiply(counts.sum(axis=1) / (10**6), axis=0)

save_df_to_npz(
    counts_per_kb, "./data/Part3_VisualCortex/GSE71585_RefSeq_CountsPerKb.npz"
)
...
counts_per_kb = load_df_from_npz(
    "./data/Part3_VisualCortex/GSE71585_RefSeq_CountsPerKb.npz"
)

celltype = pd.read_csv(
    "./data/Part3_VisualCortex/GSE71585_Clustering_Results.csv", index_col=0
)
celltype.head()

celltype["broad_type"].value_counts()

neuron_idx = celltype.index[
    celltype["broad_type"].isin(["Glutamatergic Neuron", "GABA-ergic Neuron"])
]
...
outliers = [
    "Nos1_tdTpositive_cell_72",
    "PvalbF-Gad2_tdTpositive_cell_66",
    "Vip_tdTpositive_cell_29",
    "Vip_tdTpositive_cell_61",
    "Cux2_tdTpositive_cell_35",
    "Rorb_tdTpositive_cell_13",
...
hrvatinetal_genes = load_df_from_npz(
    "./data/Part3_VisualCortex/GSE102827_merged_ExcitatoryInhibatoryNeurons_raw.npz"
)
hrvatinetal_genes = hrvatinetal_genes.columns

summary = (
    counts_per_kb.shape[1],
    len(hrvatinetal_genes),
    len(set(counts_per_kb.columns).intersection(set(hrvatinetal_genes))),
)
...
ax.hist(
    genemean.loc[numnonzero > nnzthresh].apply(np.log10),
    bins=100,
    color="r",
    label="Pass thresh",
    alpha=0.7,
...
todrop = genemean.index[
    (genemean.apply(np.log10) < logexpcutoff) | (numnonzero <= nnzthresh)
]
counts_per_kb.drop(todrop, axis=1, inplace=True)
TPM.drop(todrop, axis=1, inplace=True)
...
save_df_to_npz(
    counts_per_kb, "./data/Part3_VisualCortex/GSE71585_RefSeq_CountsPerKb.filtered.npz"
)
...
(genestats, gene_fano_parameters) = get_highvar_genes(
    TPM, numgenes=2000, minimal_mean=0
)
