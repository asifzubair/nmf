...
tpm = sc.AnnData(
    X=tpm.values, obs=pd.DataFrame(index=tpm.index), var=pd.DataFrame(index=tpm.columns)
)

highvargenes = open(highvargenesfn).read().rstrip().split("\n")
norm_counts = nmh.get_norm_counts(
    input_counts, tpm, num_highvar_genes=None, high_variance_genes_filter=highvargenes
)
...
my_plot_nmf(
    ...
    k0=15,
    k=27,
    fig_name="./nmf_cv_curve_tasic_data_r_5_k_15_26_run_1.pdf",
)
