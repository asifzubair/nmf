...
system(' !wget -O ./example_simulated_data.tar.gz https://storage.googleapis.com/sabeti-public/dkotliar/cNMF/example_data_20191024.tar.gz')
system(' !tar -zxvf ./example_simulated_data.tar.gz && rm ./example_simulated_data.tar.gz')
...
prepare_cmd = 'python ../cNMF/cnmf.py prepare --output-dir %s --name %s -c %s -k %s --n-iter %d --total-workers %d --seed %d --numgenes %d' % (output_directory, run_name, countfn, K, numiter, numworkers, seed, numhvgenes)
...
prepare_cmd = 'python ../cNMF/cnmf.py prepare --output-dir %s --name %s -c %s -k %s --n-iter %d --total-workers %d --seed %d --numgenes %d' % (output_directory, run_name, countfn, K, numiter, parallelize_numworkers, seed, numhvgenes)
...
cmd = 'nohup parallel python ../cNMF/cnmf.py factorize --output-dir %s --name %s --worker-index {} ::: %s' % (output_directory, run_name, worker_index)
print(cmd)
#!{cmd}
...
plot_K_selection_cmd = 'python ../cNMF/cnmf.py k_selection_plot --output-dir %s --name %s' % (output_directory, run_name)
...
consensus_cmd = 'python ../cNMF/cnmf.py consensus --output-dir %s --name %s --local-density-threshold %.2f --components %d --show-clustering' % (output_directory, run_name, 2.00, selected_K)
